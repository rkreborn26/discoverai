#!/usr/bin/env python3
"""
DiscoverAI: Fruits Batch Importer
===================================

Loads data/raw/fruits_products_generated.csv (396 rows, SKU951+) into Neon.
Unlike the vegetables batch, no new categories are needed here - all 6
Fruits categories (Citrus, Berries, Melons, Tropical & Exotic, Pome & Core,
Stone Fruits) already exist.

Same approach as IMPORT_FRUITS_VEGETABLES_BATCH.py:
  ...
  2. Derive and upsert product_attributes for every product: organic
     (bool), pack_size (e.g. "250g", "1kg"), and prep_type (whole / peeled /
     diced / sliced / deseeded / pitted / cut) stored in the free-form
     `attributes` json column, parsed from the product name.

Must be run on a machine that can reach your Neon DB.

Usage:
    python IMPORT_FRUITS_BATCH.py            # do the real import
    python IMPORT_FRUITS_BATCH.py --dry-run   # parse + validate only, no DB writes/connection
"""
...
CSV_PATH = Path('data/raw/fruits_products_generated.csv')

KNOWN_CATEGORY_IDS = {
    "cat_003a1",  # Citrus Fruits
    "cat_003a2",  # Berries
    "cat_003a3",  # Melons
    "cat_003a4",  # Tropical & Exotic Fruits
    "cat_003a5",  # Pome & Core Fruits
    "cat_003a6",  # Stone Fruits
}

# Checked longest-phrase-first so e.g. "Peeled & Diced" wins over "Diced".
PREP_TYPE_MARKERS = [
...
def derive_quantity_unit(pack_size):
    """Split pack_size ('250g', '1kg') into a mandatory (quantity, unit)
    pair. unit is 'gm' or 'kg'. Returns (None, None) if unparseable -
    callers must treat that as a validation error."""
    if not pack_size:
        return None, None
    s = pack_size.strip()
...
def load_and_validate(csv_path):
    if not csv_path.exists():
        ...
        print("run GENERATE_FRUITS_CSV.py first.")
        sys.exit(1)

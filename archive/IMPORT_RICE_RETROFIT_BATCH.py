#!/usr/bin/env python3
"""
DiscoverAI: Rice Retrofit Batch Importer
=============================================

Loads data/raw/rice_retrofit_products.csv (180 rows, SKU2083+) into
cat_005 "Rice" - the organic/polish variant combinations missing from the
original 68-row Rice batch. Same idempotent approach as every batch so
far: ON CONFLICT DO NOTHING, safe to re-run.

Usage:
    python IMPORT_RICE_RETROFIT_BATCH.py            # do the real import
    python IMPORT_RICE_RETROFIT_BATCH.py --dry-run   # parse + validate only, no DB writes/connection
"""
...
CSV_PATH = Path('data/raw/rice_retrofit_products.csv')
...
def load_and_validate(csv_path):
    if not csv_path.exists():
        ...
        print("Run GENERATE_RICE_RETROFIT_CSV.py first.")
        sys.exit(1)
